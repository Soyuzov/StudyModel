...
class DataLoader:
    ...
    def load_and_prepare_data(
        ...
        """
        Загружает данные MNIST, разделяет на обучающий и валидационный наборы,
        нормализует и добавляет измерение для каналов.
        ...
        # Объедение обучающего и валидационного набора обучения на всем наборе.
...
class LeNet5:
    ...
    def build_model(self) -> Sequential:
        ...
        model = Sequential(
            [
                Conv2D(
                    6, (5, 5), activation="relu", input_shape=self.input_shape
                ),
                MaxPooling2D((2, 2)),
    ...
    def train(
        ...
        Args:
            ...
            validation_data (tuple, optional): Валидационные данные.

        Returns:
            float: Метрика F1 для предсказанных классов на тестовом наборе.
        """
...
class ActiveLearner:
    ...
    def __init__(
        ...
        Args:
            ...
            y_train_full (numpy.ndarray): Метки классов для полного набора
            обучающих данных.
        """
        self.model = model
    ...
    def least_confidence(
        self, x_unlabeled: np.ndarray, num_samples: int
    ) -> np.ndarray:
        """
        Реализует стратегию активного обучения Least Confidence (LC) для выбора
        наиболее неопределенных данных.
        ...
        Примечания:
            ...
            Затем предсказываются вероятности для неразмеченных данных,
            и выбираются те, для которых модель имеет наименьшую уверенность.
        """
        # Предварительное обучение.
        self.model.fit(
            self.x_train_full[:100],
            self.y_train_full[:100],
            epochs=5,
            verbose=0,
        )
    ...
    def bald(self, x_unlabeled: np.ndarray, num_samples: int) -> np.ndarray:
        """
        Реализует стратегию активного обучения BALD (Bayesian Active Learning
        by Disagreement) для выбора данных с наибольшей неопределенностью.
        ...
        Примечания:
            ----------
            Сначала модель предварительно обучается на небольшом наборе данных.
            Затем вычисляется неопределенность для неразмеченных данных как
            сумма отрицательных произведений вероятностей и их логарифмов.
        ...
        uncertainties = np.sum(
            -predictions * np.log(predictions + 1e-9), axis=1
        )
...
print(
    f"F1 на тестовом наборе модели, обученной на всем датасете: {f1_full:.4f}"
)
...
for p in percentages:
    ...
    for _ in range(5):
        indices = random.sample(
            range(len(x_train_full)), int(len(x_train_full) * p)
        )
...
# Активное обучение.
f1_active_learning: dict[float, dict[str, list[float]]] = {
    p: {"LC": [], "BALD": []} for p in percentages
}
...
for p in percentages:
    ...
    for _ in range(5):
        ...
        active_learner_lc = ActiveLearner(
            lenet5_lc.model, x_train_full, y_train_full
        )
        indices_lc = active_learner_lc.least_confidence(
            x_train_sample, int(p * len(x_train_full))
        )
        ...
        active_learner_bald = ActiveLearner(
            lenet5_bald.model, x_train_full, y_train_full
        )
        indices_bald = active_learner_bald.bald(
            x_train_sample, int(p * len(x_train_full))
        )
...
for p, methods in f1_active_learning.items():
    ...
    for method, f1_list in methods.items():
        ...
        print(
            f"Средний F1 на тестовом наборе для {p * 100}% данных"
            f"с помощью {method}: {avg_f1:.4f}"
        )
